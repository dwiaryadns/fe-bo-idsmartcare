...
const DataTableSubscription = ({ columns, data, loading }) => {
  const {
    getTableProps,
    getTableBodyProps,
    headerGroups,
              ...
              return (
                <tr key={index} {...row.getRowProps()}>
                  {row.cells.map((cell, index) => (
                    <td key={index} {...cell.getCellProps()} style={{ padding: '10px', whiteSpace: 'nowrap' }}>
                      {cell.render('Cell')}
                    </td>
                  ))}
                </tr>
...
export default DataTableSubscription;
