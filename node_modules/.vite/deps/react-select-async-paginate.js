...
var useLazyRef = (init) => {
  const resultRef = (0, import_react10.useRef)(EMPTY_VALUE);
...
// node_modules/krustykrab/dist/esm/index.js
function Ok(result) {
  const self = {
    isOk: () => true,
...
var getInitialOptionsCache = ({
  ...
  const initialOptions = defaultOptions === true ? null : Array.isArray(defaultOptions) ? defaultOptions : options2;
  if (initialOptions) {
    return {
      "": {
        isFirstLoad: false,
...
var checkIsResponse = (response) => {
  ...
  const { options: options2, hasMore } = response;
...
var useAsyncPaginateBase = (params, deps = []) => {
  ...
  const [_stateId, setStateId] = (0, import_react13.useState)(0);
...
var useAsyncPaginate = (params, deps = []) => {
  ...
  const [menuIsOpenState, setMenuIsOpen] = (0, import_react12.useState)(!!defaultMenuIsOpenParam);
  ...
  const onInputChange = (0, import_react12.useCallback)(
...
function wrapMenuList(MenuList22) {
  function WrappedMenuList(props) {
    ...
    const setCheckAndHandleTimeout = (0, import_react15.useMemo)(() => {
      const res = () => {
        checkAndHandle();
